...
public class ReinforcementAgent implements Agent{
	...
	private final int max_episodios = 400;
			...
			else if(numRand == 2)//RIGHT
			{
				...
				if(j < col-1)//verificar limite do lado direito da tela
				{
					if(viewMatriz[i_viewMatriz][j_viewMatriz+1] != -60)//verificar se o proximo estado é uma parede
					{
						...
						if(map[i][j+1].getUp() == max_reward && map[i][j+1].getDown() == max_reward && map[i][j+1].getRight() == max_reward && map[i][j+1].getLeft() == max_reward)
							this.maior = max_reward;
						else
							this.maior = verifyMaior(up_max, right_max, left_max, down_max);
...
//				}
			...
			if(this.j == col-1)
			{
				if(map[i][j].getUp() == max_reward && map[i][j].getDown() == max_reward && map[i][j].getRight() == max_reward && map[i][j].getLeft() == max_reward)
					this.count++;
			}
...
//			maior = down_max;
//		else if(up_max == down_max && up_max == right_max && up_max == left_max)
//			maior = up_max;
//		else
//			maior = 0;
